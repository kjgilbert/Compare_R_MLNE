...
patches.y <- 10		# number of patches lined up vertically in the cylinder
...
dist.mean <- 0
dist.sd <- 10	# this is sigma, one standard deviation

par(mfrow=c(2,1))
#plot this kernel
x <- seq(-patches.x/2, patches.x/2, by=0.1)  # vector of length odd number so that there can be a middle match
...
# take the distribution up to 99% cumulative, from the middle -> 4sigma is 99.9936 percent of the distribution
cum.dist <- y2[z+1]-y2[z-1]
for(i in 1:length(y2/2)){
	if(cum.dist >= 0.9999) break
